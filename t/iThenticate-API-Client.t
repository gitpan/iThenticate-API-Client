...
BEGIN {
    $pkg = 'iThenticate::API::Client';
    use_ok( $pkg );
}

SKIP: {
    skip 'Set environment vars IT_USERNAME, IT_PASSWORD, and IT_API_URL for testing against an iThenticate API',
        129 unless ( $ENV{IT_USERNAME} && $ENV{IT_PASSWORD} && $ENV{IT_API_URL} );
...
SKIP: {
        ...
        isa_ok( $response, 'iThenticate::API::Response' );
